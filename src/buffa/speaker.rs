...
//! invariant model:
//!
//! 1. `try_new(id, parent, cluster_id, name)` enforces nil-id / nil-parent.
...
//! | `person: optional bytes` | `person: Option<Uuid7>` | 16-byte FK → `Person.id`; absent ⇒ `None`     |
...
fn speaker_error_as_buffa(e: crate::domain::aggregates::speaker::SpeakerError) -> BuffaError {
  ...
  match e {
    ...
    E::NilParent => BuffaError::MissingRequiredField("Speaker.parent"),
...
mod tests {
  ...
  fn speaker_full_roundtrip_with_voiceprint_person_and_duration() {
    ...
    let d2 = Speaker::try_from(&w).unwrap();
    assert_eq!(d, d2);
    assert_eq!(d2.person_ref(), Some(&pid));
  }
  ...
  fn speaker_wrong_length_person_fk_errors() {
    let d = Speaker::try_new(Uuid7::new(), Uuid7::new(), 0, "")
      .unwrap()
      .with_person(Uuid7::new());
    let mut w: wire::Speaker = (&d).into();
    w.person = Some(Bytes::copy_from_slice(&[0u8; 7]));
    let err = Speaker::try_from(&w).unwrap_err();
    assert!(err.is_id_wrong_length());
  }
  ...
  fn speaker_and_audio_segments_share_fks_after_roundtrip() {
    ...
    assert_eq!(spk2.person_ref(), Some(&person_id));
    assert_eq!(spk2.id_ref(), &speaker_id);
    assert_eq!(seg1b.speaker_ref(), Some(&speaker_id));
    assert_eq!(seg2b.speaker_ref(), Some(&speaker_id));
    assert_eq!(seg1b.parent_ref(), &track_id);
    assert_eq!(seg2b.parent_ref(), &track_id);
  }
}
