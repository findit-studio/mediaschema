...
//!
//! `location_path_hash` is a 32-byte `BLOB` SHA-256 of the same canonical
//! `location_path` string, computed at write time so the
//! `UNIQUE (location_volume, location_path_hash)` index can enforce
//! one-copy-per-path with the same shape that MySQL/InnoDB requires
//! (it needs a fixed-width column for `UNIQUE` over variable-length
//! `TEXT`). The hash carries no domain information — `TryFrom` only
//! verifies its length (32 bytes).
...
mod tests {
  ...
  fn media_file_rejects_wrong_hash_length() {
    ...
    let f = MediaFile::try_new(Uuid7::new(), Uuid7::new(), None, location, &wl).unwrap();
    let mut row: SqliteMediaFileRow = (&f).into();
