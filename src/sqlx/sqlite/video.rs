...
impl TryFrom<SqliteSceneRow> for Scene<Uuid7> {
  type Error = SqlxError;

  fn try_from(r: SqliteSceneRow) -> Result<Self, Self::Error> {
    let id = bytes_to_uuid7(&r.id)?;
    let parent = bytes_to_uuid7(&r.parent)?;
...
impl TryFrom<SqliteKeyframeRows> for Keyframe<Uuid7> {
  type Error = SqlxError;

  fn try_from(rows: SqliteKeyframeRows) -> Result<Self, Self::Error> {
    let row = rows
      .keyframe
...
impl<'r> TryFrom<SqliteSceneRowRef<'r>> for Scene<Uuid7> {
  type Error = SqlxError;

  fn try_from(r: SqliteSceneRowRef<'r>) -> Result<Self, Self::Error> {
    let id = bytes_to_uuid7(r.id)?;
    let parent = bytes_to_uuid7(r.parent)?;
    let index = u32_from_i64(r.index, "Scene.index")?;
    let span = crate::sqlx::dto::time_range_from_parts(
...
impl<'r> TryFrom<SqliteKeyframeRowsRef<'r>> for Keyframe<Uuid7> {
  type Error = SqlxError;

  fn try_from(rows: SqliteKeyframeRowsRef<'r>) -> Result<Self, Self::Error> {
    let row = rows
      .keyframe
      .ok_or_else(|| SqlxError::DomainConstructorRejected("Keyframe row is missing".to_owned()))?;
    let id = bytes_to_uuid7(row.id)?;
