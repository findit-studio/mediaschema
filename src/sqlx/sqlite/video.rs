...
pub struct SqliteKeyframeRow {
  ...
  pub role: String,
  pub pts: i64,
  /// FK → `thumbnail.id` (16-byte BLOB). The image bytes + storage
...
pub struct SqliteCoverKeyframeRows {
  pub cover_keyframe: Option<SqliteCoverKeyframeRow>,
  pub classifications: Vec<SqliteKeyframeClassificationRow>,
  pub objects: Vec<SqliteKeyframeObjectRow>,
  pub actions: Vec<SqliteKeyframeActionRow>,
  pub text_detections: Vec<SqliteKeyframeTextDetectionRow>,
...
/// child rows are produced by the shared [`SqliteKeyframeRows`] mapping
/// (keyed by the cover keyframe's id), so the cover analysis persists
/// exactly like a scene keyframe's.
pub fn cover_keyframe_rows_from(k: &Keyframe<Uuid7>, video_id: Uuid7) -> SqliteCoverKeyframeRows {
  // Reuse the full keyframe-rows mapping for the base scalars + every
  // detection child table.
  let base = SqliteKeyframeRows::from(k);
  let kr = base
    .keyframe
    .expect("SqliteKeyframeRows always sets the base row");
  let cover = SqliteCoverKeyframeRow {
    id: kr.id,
...
pub fn cover_keyframe_from_rows(
  ...
  let keyframe_rows = SqliteKeyframeRows {
    keyframe: Some(kr),
