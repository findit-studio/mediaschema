...
impl From<&AudioSegment<Uuid7>> for (MySqlAudioSegmentRow, Vec<MySqlAudioSegmentWordRow>) {
  fn from(s: &AudioSegment<Uuid7>) -> Self {
    ...
    let row = MySqlAudioSegmentRow {
      ...
      voice_fingerprint_dimensions: vfp.map(|v| v.dimensions()),
...
pub fn audio_segment_from_rows(
  ...
  if let Some(vid) = r.voice_fingerprint_vector_id {
    ...
    let dimensions = r.voice_fingerprint_dimensions.unwrap_or(0);
...
/// MySQL row shape for [`SoundEvent`] — the audio analog of
/// [`MySqlSceneRow`](crate::sqlx::mysql::video::MySqlSceneRow).
///
/// UUIDs ride as `BINARY(16)` (`Vec<u8>`). `span` flattens to `start_pts` /
/// `end_pts` PTS ticks only; the timebase lives on the parent `audio_track`
/// (a single stream has one timebase for all of its sound events). `code`
/// (`Option<u64>`) rides as a nullable `BIGINT`. `detector`
/// ([`CedDetector`]) rides as a `text` slug. There is no nested collection,
/// so unlike `AudioSegment` there is no child table.
#[derive(Debug, Clone, PartialEq, sqlx::FromRow)]
pub struct MySqlSoundEventRow {
  pub id: Vec<u8>,
  pub audio_track_id: Vec<u8>,
  pub index: i64,
  pub span_start_pts: i64,
...
impl From<&SoundEvent<Uuid7>> for MySqlSoundEventRow {
...
mod tests {
  ...
  fn sound_event_roundtrip_minimal() {
    ...
    .unwrap();
    let row: MySqlSoundEventRow = (&e).into();
  ...
  fn sound_event_row_with_unknown_detector_rejected() {
    ...
    .unwrap();
    let mut row: MySqlSoundEventRow = (&e).into();
