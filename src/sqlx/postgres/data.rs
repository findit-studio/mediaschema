//! PostgreSQL row shapes for the data-cluster aggregates: the `Data` facet
//! and `DataTrack` (+ the `metadata` / `index_errors` child tables).
//!
//! Identity / FK columns are native `uuid`. `codec` / `codec_tag` ride as
//! `text` slugs; `disposition` / `index_status` bitflags ride as integers.
...
/// PostgreSQL row shape for the [`Data`] facet.
#[derive(Debug, Clone, PartialEq, Eq, sqlx::FromRow)]
pub struct PgDataRow {
  pub id: Uuid,
  pub media_id: Uuid,
...
impl From<&Data<Uuid7>> for PgDataRow {
  fn from(d: &Data<Uuid7>) -> Self {
    let p = d.track_progress_ref();
    Self {
      id: uuid7_to_uuid(*d.id_ref()),
      media_id: uuid7_to_uuid(*d.media_id_ref()),
...
impl TryFrom<PgDataRow> for Data<Uuid7> {
  type Error = SqlxError;

  fn try_from(r: PgDataRow) -> Result<Self, Self::Error> {
...
/// PostgreSQL row shape for [`DataTrack`].
#[derive(Debug, Clone, PartialEq, Eq, sqlx::FromRow)]
pub struct PgDataTrackRow {
  pub id: Uuid,
  pub data_id: Uuid,
...
#[derive(Debug, Clone, PartialEq, Eq, sqlx::FromRow)]
pub struct PgDataTrackIndexErrorRow {
  pub data_track_id: Uuid,
  pub ordinal: i32,
  pub code: i32,
  pub message: String,
}

/// PostgreSQL row for `data_track_metadata`. Position in the per-
/// `data_track_id` `ordinal` sequence IS the [`IndexMap`] insertion order.
#[derive(Debug, Clone, PartialEq, Eq, sqlx::FromRow)]
pub struct PgDataTrackMetadataRow {
  pub data_track_id: Uuid,
  pub ordinal: i32,
  pub key: String,
  pub value: String,
...
impl From<&DataTrack<Uuid7>>
  for (
    PgDataTrackRow,
    Vec<PgDataTrackIndexErrorRow>,
    Vec<PgDataTrackMetadataRow>,
  )
{
  fn from(t: &DataTrack<Uuid7>) -> Self {
    ...
    let row = PgDataTrackRow {
      id,
      data_id: uuid7_to_uuid(*t.data_id_ref()),
    ...
    let errors = t
      ...
      .iter()
      .enumerate()
      .map(|(i, e)| PgDataTrackIndexErrorRow {
        data_track_id: id,
        ordinal: i as i32,
    ...
    let metadata = t
      ...
      .iter()
      .enumerate()
      .map(|(i, (k, v))| PgDataTrackMetadataRow {
        data_track_id: id,
        ordinal: i as i32,
...
pub fn data_track_from_rows(
  r: PgDataTrackRow,
  mut errors: Vec<PgDataTrackIndexErrorRow>,
  mut metadata: Vec<PgDataTrackMetadataRow>,
) -> Result<DataTrack<Uuid7>, SqlxError> {
  let id = uuid_to_uuid7(r.id)?;
  let data_id = uuid_to_uuid7(r.data_id)?;
