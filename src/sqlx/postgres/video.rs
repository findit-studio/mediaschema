...
impl<'r> TryFrom<PgSceneRowRef<'r>> for Scene<Uuid7> {
  type Error = SqlxError;

  fn try_from(r: PgSceneRowRef<'r>) -> Result<Self, Self::Error> {
    let id = uuid_to_uuid7(r.id)?;
    let parent = uuid_to_uuid7(r.parent)?;
    let index = u32_from_i64(r.index, "Scene.index")?;
    let span = crate::sqlx::dto::time_range_from_parts(
...
impl PgKeyframeRow {
  ...
  pub fn as_ref(&self) -> PgKeyframeRowRef<'_> {
    PgKeyframeRowRef {
      ...
      parent: self.parent,
      pts: self.pts,
      pts_tb_num: self.pts_tb_num,
      pts_tb_den: self.pts_tb_den,
      data: &self.data,
      mime: &self.mime,
      width: self.width,
      height: self.height,
...
impl<'r> TryFrom<PgKeyframeRowsRef<'r>> for Keyframe<Uuid7> {
  type Error = SqlxError;

  fn try_from(rows: PgKeyframeRowsRef<'r>) -> Result<Self, Self::Error> {
    let row = rows
      .keyframe
      .ok_or_else(|| SqlxError::DomainConstructorRejected("Keyframe row is missing".to_owned()))?;
    let id = uuid_to_uuid7(row.id)?;
...
mod tests {
  ...
  fn scene_ref_roundtrip() {
    ...
    .with_description("Jane is eating");
    let row: PgSceneRow = (&s).into();
    let s2: Scene<Uuid7> = row.as_ref().try_into().unwrap();
    assert_eq!(s, s2);
  }
  ...
  fn keyframe_ref_roundtrip_minimal() {
    ...
    .unwrap();
    let rows: PgKeyframeRows = (&kf).into();
    let kf2: Keyframe<Uuid7> = rows.as_ref().try_into().unwrap();
    assert_eq!(kf, kf2);
  }
