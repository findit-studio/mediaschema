...
/// lives on the parent `video_track` (a single stream has one timebase
/// for all of its scenes + keyframes).
#[derive(Debug, Clone, PartialEq, Eq, sqlx::FromRow)]
...
/// Rebuild a [`Scene`] from a stored row. The `parent_timebase` is the
/// per-stream timebase carried on the parent `video_track` row — see the
/// audio cluster's `audio_segment_from_rows` for the same pattern.
...
/// parent `video_track` (a single stream has one timebase for all of its
/// scenes + keyframes).
#[derive(Debug, Clone, PartialEq, sqlx::FromRow)]
...
/// child rows. The `parent_timebase` is the per-stream timebase carried
/// on the parent `video_track` row — see the audio cluster's
...
//
// `PgVideoRow`, `PgKeyframeSaliencyRow`, `PgKeyframeDocumentSegmentRow`,
// `PgKeyframeFaceRow`, `PgKeyframeBodyPoseRow`, `PgKeyframeHandPoseRow`,
// `PgKeyframeBodyPose3DRow`, `PgKeyframeFaceLandmarksRow`, and
// `PgKeyframeFaceLandmarkPointRow` are all-`Copy` (Uuid + integers/floats
// only), so they have no `Ref` sibling — the borrowed view would be
// structurally identical.
// ===========================================================================

/// Borrowed view of [`PgVideoTrackRow`] — zero-copy decode from `&'r Row`.
///
/// Variable-length text/byte columns borrow from the underlying row;
/// promotion to the domain [`VideoTrack`] only allocates IF the caller
/// asks for it via `TryFrom`. See [`PgVideoTrackRow::as_ref`] for the
/// cheap-borrow path from an already-owned row.
#[derive(Debug, Clone, PartialEq, sqlx::FromRow)]
pub struct PgVideoTrackRowRef<'r> {
  pub id: Uuid,
  pub video_id: Uuid,
  pub stream_index: Option<i64>,
  pub container_track_id: Option<i64>,
...
impl PgVideoTrackRow {
  /// Cheap borrow — produces a [`PgVideoTrackRowRef`] referencing `self`.
  pub fn as_ref(&self) -> PgVideoTrackRowRef<'_> {
    PgVideoTrackRowRef {
      id: self.id,
...
impl PgVideoTrackIndexErrorRow {
...
impl PgSceneRow {
  /// Cheap borrow — produces a [`PgSceneRowRef`] referencing `self`.
...
/// Borrowed view of [`PgKeyframeRow`].
#[derive(Debug, Clone, PartialEq, sqlx::FromRow)]
...
impl PgKeyframeRow {
  /// Cheap borrow — produces a [`PgKeyframeRowRef`] referencing `self`.
...
impl PgKeyframeClassificationRow {
  /// Cheap borrow.
...
impl PgKeyframeTextDetectionRow {
  /// Cheap borrow.
...
impl PgKeyframeBodyPoseJointRow {
  /// Cheap borrow.
...
impl PgKeyframeBodyPose3DJointRow {
  /// Cheap borrow.
...
impl PgKeyframeFaceLandmarkRegionRow {
  /// Cheap borrow.
...
/// Borrowed bundle for a single [`Keyframe`] — the scalar
/// [`PgKeyframeRowRef`] plus every per-detection child slice as Refs in
/// lockstep with [`PgKeyframeRows`]. Variable-length text/byte columns
/// borrow from the underlying owned bundle; only the `Vec` of refs is
/// allocated. Promotion to the domain [`Keyframe`] only allocates IF the
/// caller asks for it via `TryFrom`.
#[derive(Debug, Clone, Default, PartialEq)]
...
impl PgKeyframeRows {
  /// Cheap borrow — produces a [`PgKeyframeRowsRef`] referencing `self`.
  /// All-`Copy` child row types are cloned (cheap memcpy); the
  /// variable-length-column children are borrowed.
  pub fn as_ref(&self) -> PgKeyframeRowsRef<'_> {
    PgKeyframeRowsRef {
      keyframe: self.keyframe.as_ref().map(PgKeyframeRow::as_ref),
      classifications: self
        .classifications
        .iter()
        .map(PgKeyframeClassificationRow::as_ref)
        .collect(),
      objects: self
        .objects
        .iter()
        .map(PgKeyframeObjectRow::as_ref)
        .collect(),
      actions: self
        .actions
        .iter()
        .map(PgKeyframeActionRow::as_ref)
        .collect(),
      text_detections: self
        .text_detections
        .iter()
        .map(PgKeyframeTextDetectionRow::as_ref)
        .collect(),
      barcodes: self
        .barcodes
        .iter()
        .map(PgKeyframeBarcodeRow::as_ref)
        .collect(),
      saliencies: self.saliencies.clone(),
      document_segments: self.document_segments.clone(),
      colors: self.colors.iter().map(PgKeyframeColorRow::as_ref).collect(),
      subjects: self
        .subjects
        .iter()
        .map(PgKeyframeSubjectRow::as_ref)
        .collect(),
      ...
      body_pose_joints: self
        .body_pose_joints
        .iter()
        .map(PgKeyframeBodyPoseJointRow::as_ref)
        .collect(),
      hand_poses: self.hand_poses.clone(),
      body_poses_3d: self.body_poses_3d.clone(),
      body_pose_3d_joints: self
        .body_pose_3d_joints
        .iter()
        .map(PgKeyframeBodyPose3DJointRow::as_ref)
        .collect(),
      masks: self.masks.iter().map(PgKeyframeMaskRow::as_ref).collect(),
      face_landmarks: self.face_landmarks.clone(),
      face_landmark_regions: self
        .face_landmark_regions
        .iter()
        .map(PgKeyframeFaceLandmarkRegionRow::as_ref)
        .collect(),
      face_landmark_points: self.face_landmark_points.clone(),
      vlm_labels: self
        .vlm_labels
        .iter()
        .map(PgKeyframeVlmLabelRow::as_ref)
        .collect(),
    }
  }
}

/// Rebuild a [`Keyframe`] from a borrowed bundle of keyframe + detection
/// child rows — see [`keyframe_from_rows`] for the owned counterpart.
/// `parent_timebase` is the per-stream timebase carried on the parent
/// `video_track` row.
pub fn keyframe_from_rows_ref<'r>(
  rows: PgKeyframeRowsRef<'r>,
  parent_timebase: mediatime::Timebase,
) -> Result<Keyframe<Uuid7>, SqlxError> {
  {
    let row = rows
