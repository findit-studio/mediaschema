...
/// Borrowed view of [`PgSubtitleCueRow`].
#[derive(Debug, Clone, PartialEq, Eq, sqlx::FromRow)]
pub struct PgSubtitleCueRowRef<'r> {
  pub id: Uuid,
  pub parent: Uuid,
  pub index: i64,
  pub span_start_pts: i64,
...
impl PgSubtitleCueRow {
  /// Cheap borrow — produces a [`PgSubtitleCueRowRef`] referencing `self`.
  pub fn as_ref(&self) -> PgSubtitleCueRowRef<'_> {
    PgSubtitleCueRowRef {
      id: self.id,
      parent: self.parent,
      index: self.index,
...
impl<'r> TryFrom<PgSubtitleCueRowRef<'r>> for SubtitleCue<Uuid7> {
  type Error = SqlxError;

  fn try_from(r: PgSubtitleCueRowRef<'r>) -> Result<Self, Self::Error> {
    let id = uuid_to_uuid7(r.id)?;
    let parent = uuid_to_uuid7(r.parent)?;
    let index = u32_from_i64(r.index, "SubtitleCue.index")?;
    let span = time_range_from_parts(
