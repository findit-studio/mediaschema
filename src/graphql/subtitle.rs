...
pub struct GqlSubtitleIndexProgress(pub crate::domain::vo::IndexProgress);

impl From<crate::domain::vo::IndexProgress> for GqlSubtitleIndexProgress {
  #[inline]
  fn from(v: crate::domain::vo::IndexProgress) -> Self {
    Self(v)
  }
}
impl From<GqlSubtitleIndexProgress> for crate::domain::vo::IndexProgress {
  #[inline]
...
impl GqlSubtitle {
  ...
  async fn track_progress(&self) -> GqlSubtitleIndexProgress {
    GqlSubtitleIndexProgress(*self.0.track_progress_ref())
  }
}
...
impl GqlSubtitleTrack {
  ...
  async fn is_image_based(&self) -> Option<bool> {
    self.0.image_based()
  }
  ...
  async fn cues(&self) -> std::vec::Vec<ID> {
    self.0.cues_slice().iter().map(|id| ID(id.to_string())).collect()
  }
  ...
  async fn source_path(&self) -> Option<GqlLocation> {
    self.0.source_path_ref().cloned().map(Into::into)
  }
  async fn source_checksum(&self) -> Option<FileChecksum> {
    self.0.source_checksum_ref().copied()
  }
  ...
  async fn first_cue(&self) -> Option<GqlMediaTimestamp> {
    self.0.first_cue_ref().copied().map(GqlMediaTimestamp)
  }
  async fn last_cue(&self) -> Option<GqlMediaTimestamp> {
    self.0.last_cue_ref().copied().map(GqlMediaTimestamp)
  }
...
impl GqlSubtitleCue {
  ...
  async fn ocr_text(&self) -> GqlLocalizedText {
    GqlLocalizedText(self.0.ocr_text_ref().clone())
  }
...
mod tests {
  ...
  fn subtitle_facet_roundtrips() {
    ...
    assert_eq!(back.tracks_slice().len(), s.tracks_slice().len());
  }
  ...
  fn subtitle_cue_roundtrips() {
    ...
    let c = SubtitleCue::try_new_text(
      ...
      0,
      span,
      crate::domain::vo::LocalizedText::from_src("cue"),
    )
    .unwrap();
