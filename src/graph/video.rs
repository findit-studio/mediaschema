...
impl Video<Uuid7> {
  ...
  pub fn try_from_flat(
    ...
    Ok(Self {
      id: *facet.id_ref(),
      total_scenes: facet.total_scenes(),
...
impl VideoTrack<Uuid7> {
  ...
  pub fn try_from_flat(
    ...
    Ok(Self {
      ...
      profile: track.profile().map(SmolStr::new),
      level: track.level(),
      bit_rate: track.bit_rate(),
      nb_frames: track.nb_frames(),
      has_b_frames: track.has_b_frames(),
      closed_gop: track.closed_gop(),
      bits_per_raw_sample: track.bits_per_raw_sample(),
      dimensions: track.dimensions(),
      visible_rect: track.visible_rect(),
      sample_aspect_ratio: track.sample_aspect_ratio(),
      pixel_format: track.pixel_format(),
      color: *track.color_ref(),
      hdr_static: track.hdr_static_ref().cloned(),
      rotation: track.rotation(),
      frame_rate: track.frame_rate(),
      avg_frame_rate: track.avg_frame_rate(),
      field_order: track.field_order(),
      stereo_mode: track.stereo_mode(),
      dovi: track.dovi(),
      has_embedded_captions: track.has_embedded_captions(),
...
impl Scene<Uuid7> {
  ...
  pub fn try_from_flat(
    ...
    Ok(Self {
      ...
      keyframes,
      description: SmolStr::new(scene.description()),
    })
  }
}
...
impl Keyframe<Uuid7> {
  ...
  pub fn try_from_flat(
    ...
    parent_check(
      NodeKind::Keyframe,
      *keyframe.id_ref(),
      keyframe.scene_id_ref(),
      expected_scene,
    )?;
    Ok(Self {
      id: *keyframe.id_ref(),
      pts: *keyframe.pts_ref(),
      data: keyframe.data_bytes(),
      mime: SmolStr::new(keyframe.mime()),
      dimensions: keyframe.dimensions(),
      extractor: keyframe.extractor(),
      classifications: keyframe.classifications_slice().to_vec(),
      objects: keyframe.objects_slice().to_vec(),
      humans: keyframe.humans_ref().clone(),
      animals: keyframe.animals_ref().clone(),
      actions: keyframe.actions_slice().to_vec(),
      text_detections: keyframe.text_detections_slice().to_vec(),
      barcodes: keyframe.barcodes_slice().to_vec(),
      attention_saliency: keyframe.attention_saliency_slice().to_vec(),
      objectness_saliency: keyframe.objectness_saliency_slice().to_vec(),
      horizon: *keyframe.horizon_ref(),
      document_segments: keyframe.document_segments_slice().to_vec(),
      aesthetics: *keyframe.aesthetics_ref(),
      colors: keyframe.colors_slice().to_vec(),
      vlm: keyframe.vlm_ref().clone(),
    })
  }
}
