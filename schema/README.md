...
- **Identity = structs generic over the id type** — `MediaMeta<Id>`,
  ...
  creation time — a future API projection may expose an opaque id; this does
...
- **Embeddings live in LanceDB, not the domain (user decision).** All
  ...
  sqlx/mongodb projections have **no** vector column; a future API projection
  exposes similarity as a LanceDB-backed endpoint keyed by `id` (never a raw
  vector). Cancels the earlier shared-`Embedding`-VO idea.
...
- **Conversion sequencing:** the domain types (generic over `Id`) are designed
  ...
  are **deferred until the projections are defined**, then written once —
...
- **Indexing errors/status are API-facing, not internal:** `index_errors`
  ...
  the API projection** on every aggregate (clients display why/whether
...
- [ ] [sound_events.md](sound_events.md) — `SoundEvent` (per-track CED sound-event detection) — **drafted (rev 1)** — audio analog of `Scene`; `span: TimeRange`, `label`, `code: Option<u64>`, `score: f32`, `CedDetector`; flat aggregate + graph leaf; sqlx/mongodb/buffa bridges + `AudioTrack` rollup deferred
