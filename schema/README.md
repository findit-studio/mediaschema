...
- [ ] [sound_events.md](sound_events.md) — `SoundEvent` (per-track CED sound-event detection) — **drafted (rev 1)** — audio analog of `Scene`; `span: TimeRange`, `label`, `code: Option<u64>`, `score: f32`; flat aggregate + graph leaf; sqlx/mongodb/buffa bridges + `AudioTrack` rollup deferred
