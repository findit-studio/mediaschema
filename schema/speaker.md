# `Speaker<Id>` — a per-track diarized speaker  *(rev 2 — drafted, in-review)*
...
- **sqlx**: `speaker` table; `id` PK; `parent` FK → `audio_track`;
  ...
  **flattened**: `voiceprint_vector_id` (`uuid`, **discriminator** — `NOT
  ...
  columns. Nullable `person` FK column with `idx_speaker_person ON
  speaker(person)` for FK lookup from a `Person`. `(parent, cluster_id)`
  unique.
- **mongodb**: `_id`=UUIDv7; single `speakers` collection; `voiceprint`
  ...
  sub-docs, so flattening is unnecessary); `person` field with single-field
  index `speakers_person` for FK lookup from `persons`.
