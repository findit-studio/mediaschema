...
Change from rev 7: `scenes: Vec<Id>` **removed** → moved to
`VideoTrack.scenes` (per-track). No scalar metadata of its own; file-level →
`Media`, stream/scene-level → `VideoTrack`.

## Projection notes

- **sqlx**: `video` table = `id` PK (uuid); `total_scenes` column; `tracks`
