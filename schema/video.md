# `Video<Id>` — video facet (thin aggregate)  *(rev 9 — LOCKED, user-approved; `parent` FK to Media)*
...
`Subtitle.parent`; foundation for sqlx/mongodb schema additions). Change
...
- **sqlx**: `video` table = `id` PK (uuid); `parent` uuid FK → `media.id`;
...
(Audio/Video/Subtitle) share a uniform back-reference; additive on top
of `Media.video`.)*
